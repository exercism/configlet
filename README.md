...
You can run `configlet generate` to generate the exercise's `introduction.md` for any exercise that has an `introduction.md.tpl` file. The generated `introduction.md` file is identical to the `introduction.md.tpl` file, except for concept placeholders, which are replaced with the contents of the concept's `introduction.md` file (minus its top-level heading and with its other headings' level incremented by one).
